	...
	.handler(async ({ data }) => {
		...
		if (data.role) {
			filteredUsers = filteredUsers.filter((user) =>
				user.user_roles?.some((r: { role: string }) => r.role === data.role),
			);
		}
...
export const updateUserStatus = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		if (!updatedRows?.length) {
			throw new Error(
				"No profile was updated (missing user or RLS). Super admins need the profiles UPDATE policy — apply latest Supabase migrations.",
			);
		}
