...
import { Building, Check, Eye, Filter, Target } from "lucide-react";
import {
...
export const Route = createFileRoute("/_authenticated/dashboard/campaigns/")({
	...
	): { status: CampaignStatus | undefined; page: number } => ({
...
function CampaignsPage() {
	...
	const handleStatusFilter = (newStatus: string) => {
		navigate({
			search: {
				status: newStatus === "all" ? undefined : (newStatus as CampaignStatus),
				page: 1,
			},
		});
	};
						...
						>
							...
							<div className="p-4">
								...
								<div className="flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 mt-1">
									...
									{getLocalizedName(
										(campaign as { churches?: { name?: unknown } }).churches
											?.name,
									)}
								</div>
