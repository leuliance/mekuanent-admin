						...
						<div className="rounded-xl border bg-card p-5">
							...
							<div className="grid grid-cols-3 gap-4">
								<div className="text-center p-3 rounded-lg bg-muted/50">
									...
									<p className="text-lg font-bold">{item.view_count || 0}</p>
									<p className="text-xs text-muted-foreground">Views</p>
								</div>
								<div className="text-center p-3 rounded-lg bg-muted/50">
									<Heart className="h-5 w-5 mx-auto mb-1 text-red-500" />
									<p className="text-lg font-bold">{item.like_count || 0}</p>
									<p className="text-xs text-muted-foreground">Likes</p>
								</div>
								<div className="text-center p-3 rounded-lg bg-muted/50">
									<Share2 className="h-5 w-5 mx-auto mb-1 text-green-500" />
									<p className="text-lg font-bold">{item.share_count || 0}</p>
								...
								<div className="mt-4 pt-4 border-t">
									...
									<div className="space-y-1 text-sm">
										{item.video_content.duration_seconds && (
											<p>
												Duration:{" "}
												{Math.floor(item.video_content.duration_seconds / 60)}m{" "}
												{item.video_content.duration_seconds % 60}s
											</p>
										)}
								...
								<div className="mt-4 pt-4 border-t">
									...
									<div className="space-y-1 text-sm">
										{item.audio_content.duration_seconds && (
											<p>
												Duration:{" "}
												{Math.floor(item.audio_content.duration_seconds / 60)}m{" "}
												{item.audio_content.duration_seconds % 60}s
											</p>
										)}
							...
							{item.content_type === "article" && item.article_content && (
								<div className="mt-4 pt-4 border-t">
									<h3 className="text-xs font-semibold text-muted-foreground mb-2">
										Article Details
									</h3>
									<div className="space-y-1 text-sm">
										{item.article_content.read_time_minutes && (
											<p>
												Read time: {item.article_content.read_time_minutes} min
											</p>
										)}
