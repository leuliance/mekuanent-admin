...
export const Route = createFileRoute("/_authenticated/dashboard/bible/")({
	validateSearch: (
		...
		testament: search.testament ? (search.testament as Testament) : undefined,
			...
			<div className="flex-1 flex items-center justify-center p-4">
				<div className="text-center max-w-md">
					...
					<h2 className="text-xl font-semibold mb-2">
						Failed to Load Bible Books
					</h2>
					<p className="text-muted-foreground mb-4">
						{error.message ||
							"An unexpected error occurred while loading the Bible content."}
					</p>
					<Button
						onClick={() =>
							navigate({
...
function BibleBooksPage() {
	...
	const handleTestamentFilter = (newTestament: string) => {
		navigate({
			search: {
				testament:
					newTestament === "all" ? undefined : (newTestament as Testament),
				page: 1,
				search: searchInput,
			},
		});
					...
					<>
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
							{typedBooks.map((book) => {
								...
								const isOldTestament = testamentText
									.toLowerCase()
									.includes("old");
						...
						{totalPages > 1 && (
							<div className="flex items-center justify-between">
								<p className="text-sm text-muted-foreground">
									Showing {(page - 1) * 66 + 1} to {Math.min(page * 66, total)}{" "}
									of {total} books
								</p>
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							...
							{bookToDelete && getLocalizedName(bookToDelete.name)}"? This will
							also delete all chapters and verses in this book. This action
							cannot be undone.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
							...
							{(field) => (
								<div className="space-y-2">
									...
									<Select
										value={field.state.value}
										onValueChange={(value) =>
											field.handleChange(value as "old" | "new")
										}
									>
										<SelectTrigger>
