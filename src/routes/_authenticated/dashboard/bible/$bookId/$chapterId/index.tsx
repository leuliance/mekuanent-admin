...
import type { BibleBook, BibleChapter, BibleVerse, BibleFootnote, BibleCrossReference } from "@/api/bible";
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <div className="flex gap-2 justify-center">
            <Button
              variant="outline"
              onClick={() => navigate({ to: "/dashboard/bible", search: { testament: undefined, page: 1, search: "" } })}
            >
...
function ChapterDetailPage() {
  const { chapter, verses, total, page, totalPages } = Route.useLoaderData();
  ...
  const [crossRefs, setCrossRefs] = useState<Record<string, BibleCrossReference[]>>({});
  ...
  const [selectedVerseForAdd, setSelectedVerseForAdd] = useState<BibleVerse | null>(null);
  ...
  const [editingFootnote, setEditingFootnote] = useState<BibleFootnote | null>(null);
  const [editingFootnoteVerseId, setEditingFootnoteVerseId] = useState<string | null>(null);
  ...
  const createCrossRefForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        const verseRange = value.reference_verse_end && value.reference_verse_end > 0
        ...
        await createBibleCrossReference({
          data: {
            ...
            reference_verse_end: value.reference_verse_end && value.reference_verse_end > 0 ? value.reference_verse_end : undefined,
          },
        });
  ...
  const loadFootnotesForVerse = async (verseId: string) => {
    ...
    try {
      ...
      setFootnotes((prev) => ({ ...prev, [verseId]: footnotesResult as BibleFootnote[] }));
      setCrossRefs((prev) => ({ ...prev, [verseId]: crossRefsResult as BibleCrossReference[] }));
    } catch (error) {
  ...
  const loadCrossRefsForVerse = async (verseId: string) => {
    try {
      ...
      setCrossRefs((prev) => ({ ...prev, [verseId]: result as BibleCrossReference[] }));
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto p-4">
        {/* Back Button */}
        <Button variant="ghost" size="sm" className="mb-4" render={<Link to="/dashboard/bible/$bookId" params={{ bookId }} search={{ page: 1 }} />}>
          <Link to="/dashboard/bible/$bookId" params={{ bookId }} search={{ page: 1 }}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to {bookName}
          </Link>
        </Button>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => setCreateVerseDialogOpen(true)}>
            <Plus className="w-4 h-4 mr-2" />
            Add Verse
          </Button>
        </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 space-y-3">
                            <div>
                              <Label htmlFor="text-en" className="text-xs text-muted-foreground">
                                English
                              </Label>
                              <Textarea
                                id="text-en"
                            ...
                            <div>
                              <Label htmlFor="text-am" className="text-xs text-muted-foreground">
                                Amharic
                              </Label>
                              <Textarea
                                id="text-am"
                                    ...
                                    <div className="space-y-2">
                                      {footnotes[verse.id]?.map((footnote) => (
                                        ...
                                        >
                                          ...
                                          <span>{getLocalizedText(footnote.note, "en")}</span>
                                          ...
                                          <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button
                                              ...
                                              size="icon"
                                              className="h-6 w-6"
                                              onClick={() => handleEditFootnote(footnote, verse.id)}
                                            >
                                              ...
                                              <Button
                                                ...
                                                size="icon"
                                                className="h-6 w-6"
                                                onClick={() => handleDeleteFootnote(footnote.id, verse.id)}
                                              >
                                  ...
                                  {crossRefs[verse.id]?.length === 0 ? (
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {crossRefs[verse.id]?.map((ref) => {
                                        ...
                                        const joinedBook = (ref as { bible_books?: BibleBook }).bible_books;
                                        ...
                                        const verseRange = ref.reference_verse_end
                                          ? `${ref.reference_verse_start}-${ref.reference_verse_end}`
                                          : String(ref.reference_verse_start || "");
                                        return (
                                          <div
                                            key={ref.id}
                                          ...
                                          >
                                            <span>
                                              {refBookName} {ref.reference_chapter}:{verseRange}
                                            </span>
                                              <Button
                                                variant="ghost"
                                                size="icon"
                                                className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity"
                                                onClick={() => handleDeleteCrossRef(ref.id, verse.id)}
                                              >
                                                <X className="w-3 h-3" />
                                              </Button>
                                          </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Showing {(page - 1) * 200 + 1} to {Math.min(page * 200, total)} of{" "}
                  {total} verses
                </p>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete verse {verseToDelete?.verse_number}? This action
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      {/* Create Verse Dialog */}
      <Dialog open={createVerseDialogOpen} onOpenChange={setCreateVerseDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add New Verse</DialogTitle>
            <DialogDescription>
              Add a new verse to {bookName} Chapter {typedChapter.chapter_number}
            </DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
      ...
      {/* Add Footnote Dialog */}
      <Dialog open={addFootnoteDialogOpen} onOpenChange={setAddFootnoteDialogOpen}>
        <DialogContent>
          <DialogHeader>
      ...
      <Dialog open={editFootnoteDialogOpen} onOpenChange={setEditFootnoteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Edit Footnote</DialogTitle>
            <DialogDescription>
              Update the footnote details
            </DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
      ...
      <Dialog open={addCrossRefDialogOpen} onOpenChange={setAddCrossRefDialogOpen}>
        <DialogContent>
          <DialogHeader>
