      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-4">
            {error.message || "An unexpected error occurred while loading the book."}
          </p>
          <div className="flex gap-2 justify-center">
            <Button variant="outline" onClick={() => navigate({ to: "/dashboard/bible", search: { testament: undefined, page: 1, search: "" } })}>
...
function ChaptersEmptyState({ bookName, onAddChapter }: { bookName: string; onAddChapter: () => void }) {
  return (
    <div className="flex-1 flex items-center justify-center p-4">
      <div className="text-center max-w-md">
...
function BookDetailPage() {
  ...
  const [chapterToDelete, setChapterToDelete] = useState<BibleChapter | null>(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate({ to: "/dashboard/bible/$bookId", params: { bookId }, search: { page } });
    } catch (error) {
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto p-4">
        ...
        <Button
          ...
          className="mb-4"
          render={
            <Link to="/dashboard/bible" search={{ testament: undefined, page: 1, search: "" }} />
          }
          nativeButton={false}
        >
          <ArrowLeft className="w-4 h-4 mr-2" />
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                {testamentText} • Book #{typedBook.book_number} • {typedBook.chapter_count} chapters
              </p>
            </div>
          </div>
        ...
        {typedChapters.length === 0 ? (
          <ChaptersEmptyState bookName={bookName} onAddChapter={() => setCreateDialogOpen(true)} />
        ) : (
          <>
            {/* Chapters Grid */}
            <div className="mb-4">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Showing {(page - 1) * 150 + 1} to {Math.min(page * 150, total)} of{" "}
                  {total} chapters
                </p>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete Chapter {chapterToDelete?.chapter_number}?
