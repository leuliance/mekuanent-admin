...
function EventDetailPage() {
	...
	const startEditing = () => {
		...
		setEditRsvpDeadline(
			item.rsvp_deadline ? formatDateTimeLocal(item.rsvp_deadline) : "",
		);
		setIsEditing(true);
	};
				...
				<div className="max-w-4xl mx-auto space-y-6">
					...
					{isEditing ? (
						...
						<>
							...
							{/* RSVPs & optional donation tab */}
							{item.has_donation ? (
								<ResponsiveTabs
									value={eventTab}
									onValueChange={setEventTab}
									className="rounded-xl border bg-card"
									selectPlaceholder="Event data"
									listClassName="flex w-full flex-wrap gap-1 border-b px-5 py-3 bg-transparent"
									items={[
										{
											value: "rsvps",
											label: `RSVPs (${rsvps.length})`,
											trigger: (
												<>
										...
										{
											value: "donations",
											label: `Donations (${(donations as unknown[]).length})`,
											trigger: (
												<>
...
function mergeLocalDateAndTime(date: Date, timeHHmm: string): string {
	const [hhRaw, mmRaw] = timeHHmm.split(":");
	const h = Number.parseInt(hhRaw ?? "0", 10);
	const m = Number.parseInt(mmRaw ?? "0", 10);
	const pad = (n: number) => String(n).padStart(2, "0");
	return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(Number.isFinite(h) ? h : 0)}:${pad(Number.isFinite(m) ? m : 0)}`;
}
	...
	return (
		<div className="space-y-1.5">
			...
			<Popover>
				<PopoverTrigger
					render={
						<Button
							type="button"
							variant="outline"
							className="h-10 w-full min-w-0 justify-start px-3 text-left font-normal"
						/>
					}
					nativeButton={false}
				>
				...
				<PopoverContent
					className="w-auto max-w-[min(100vw-2rem,320px)] gap-0 overflow-hidden p-0"
					align="start"
				>
					<DayPickerCalendar
						mode="single"
						...
						onSelect={(day) => {
							if (!day) return;
							onChange(mergeLocalDateAndTime(day, timeValue));
						}}
					/>
					<div className="flex flex-wrap items-end gap-2 border-t p-3">
						<div className="min-w-0 flex-1 space-y-1">
							...
							<Input
								...
								onChange={(e) =>
									onChange(
										mergeLocalDateAndTime(baseDateForTime, e.target.value),
									)
								}
							/>
						</div>
		...
		<div className="space-y-4">
			...
			<div className="rounded-xl border bg-card p-5">
				...
				<div className="flex items-center gap-2 mb-4">
					<Switch checked={editIsOnline} onCheckedChange={setEditIsOnline} />
