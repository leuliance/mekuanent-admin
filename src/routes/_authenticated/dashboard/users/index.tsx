...
import { ALL_ROLES, canBanUsers, ROLE_COLORS, ROLE_LABELS } from "@/lib/roles";
...
function UsersPage() {
	...
	const [statusTargetUser, setStatusTargetUser] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	}, [searchInput, searchParams.search, debouncedSearch.maybeExecute]);
				...
				<div className="mx-auto max-w-7xl space-y-6">
					...
					<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
						...
						<div className="flex gap-2 flex-wrap">
							...
							<div className="px-3 py-1 bg-orange-100 dark:bg-orange-900/30 rounded-full text-sm font-medium text-orange-700 dark:text-orange-400">
								{stats.admins} Admins
							</div>
								...
								<table className="w-full">
									<thead className="bg-muted/50">
										<tr>
											...
											<th className="min-w-48 px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider sm:min-w-56">
												Roles
											</th>
											<th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">
												Status
						...
						<div className="bg-card rounded-xl border overflow-hidden">
							<div className="overflow-x-auto">
								<table className="w-full">
									...
									<tbody className="divide-y divide-border">
										{users.map((user) => {
											const userStatus = getUserStatusFromData(
											...
											return (
												...
												>
													...
													<td className="min-w-48 max-w-[min(100%,20rem)] align-top px-4 py-4 sm:min-w-56">
														<div className="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap">
															{user.user_roles && user.user_roles.length > 0 ? (
																user.user_roles.map(
																	(ur: { role: UserRole }, idx: number) => (
																		<span
																			key={idx}
																			className={`inline-flex w-fit max-w-full shrink-0 whitespace-nowrap rounded-full px-2.5 py-0.5 text-xs font-medium ${ROLE_COLORS[ur.role] ?? ROLE_COLORS.viewer}`}
																		>
																...
																<span
																	className={`inline-flex w-fit rounded-full px-2.5 py-0.5 text-xs font-medium ${ROLE_COLORS.viewer}`}
																>
																	No role
																</span>
															)}
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							...
							{(statusTargetUser?.first_name as string) || ""}{" "}
							{(statusTargetUser?.last_name as string) || ""}.
						</DialogDescription>
					</DialogHeader>
					<div className="space-y-4 py-2">
						<div className="space-y-2">
							...
							<Select
								value={newStatus}
								onValueChange={(v) => setNewStatus(v as UserAccountStatus)}
							>
								<SelectTrigger>
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
									...
									<SelectItem value="inactive">
										<span className="flex items-center gap-2">
											<UserX className="w-3.5 h-3.5 text-gray-600" /> Inactive
										</span>
									</SelectItem>
