...
export const Route = createFileRoute("/_authenticated/dashboard/churches/")({
	validateSearch: (
		...
		category: search.category ? (search.category as ChurchCategory) : undefined,
	...
	const handleCategoryFilter = (value: string | null) => {
		navigate({
			search: (prev) => ({
				...prev,
				category:
					!value || value === "all" ? undefined : (value as ChurchCategory),
				page: 1,
			}),
		});
	};
												...
												<td className="px-4 py-3">
													<div className="flex items-center gap-1.5 text-sm text-muted-foreground">
														...
														<span className="truncate max-w-[120px]">
															{getLocalizedText(church.city, locale) ||
																"Unknown"}
														</span>
													</div>
												</td>
							...
							{totalPages > 1 && (
								<div className="px-4 py-3 border-t flex items-center justify-between">
									<p className="text-sm text-muted-foreground">
										...
										{Math.min(page * 10, total)} of {total} churches
									</p>
									<div className="flex gap-2">
										<Button
											variant="outline"
