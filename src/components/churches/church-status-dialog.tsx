...
import { type Church, updateChurchStatus } from "@/api/churches";
		...
		try {
			await updateChurchStatus({
				data: {
					...
					status: targetStatus,
					rejected_reason:
						targetStatus === "rejected" ? rejectedReason : undefined,
				},
			});
			onOpenChange(false);
			setRejectedReason("");
			...
			<DialogContent>
				...
				{targetStatus === "rejected" && (
					<div className="space-y-2">
						<label className="text-sm font-medium" htmlFor="rejectedReason">
				...
				<DialogFooter>
					...
					<Button
						...
						disabled={
							isUpdating || (targetStatus === "rejected" && !rejectedReason)
						}
					>
						{isUpdating ? (
							<>
