...
export function EventDonationsTable({
	donations,
}: {
		...
		<div className="space-y-4">
			...
			<div className="flex items-center gap-2 text-sm">
				...
				<span className="text-muted-foreground">
					raised from {donations.length} donation
					...
					<tbody className="divide-y">
						{donations.map((donation) => (
							<tr key={donation.id} className="hover:bg-muted/30">
								...
								<td className="px-3 py-2.5">
									<span
										className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${donationStatusColors[donation.status] || ""}`}
									>
										{donation.status}
									</span>
								</td>
